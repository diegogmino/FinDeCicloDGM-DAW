...
import PaginationComponent from "../pagination/PaginationComponent"
  ...
  export default function FilmsGallery(props) {
    ...
    const {films, setPage, numberPages, filmsNumber} = props;
            ...
            {films.map((film) => (
              <div key={film.id} className="group relative">
                ...
                <div className="mt-4 flex justify-between">
                  ...
                  <p className="text-sm font-medium text-gray-900">{film.precio}€</p>
                </div>
              </div>
            ))}
          </div>
        </div>
        <PaginationComponent setPage={setPage} numberPages={numberPages} filmsNumber={filmsNumber}/>
      </div>
    )
  }
